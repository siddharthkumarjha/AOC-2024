...
const string = splitByLine.string;
const lines = splitByLine.lines;
...
pub fn main() !void {
    const fstream = try cwd.openFile("./input.txt", .{});
    defer fstream.close();
    ...
    while (MoveSoldier(&array_of_lines, &pos, direction) != .OutOfBounds) {
        std.log.info("Soldier collided at line: {}, col: {}", pos);
        direction = RotateRight(direction);
    } else {
        std.log.info("Soldier went OOB at line: {}, col: {}", pos);
    }
    std.log.debug("\n{s}\n", .{buffer.items});
    std.log.info("======================================================", .{});
    std.log.info("no of distinct positions visited: {}", .{DistinctPosVisited(&buffer)});
}
