...
pub const MoveResult = enum { OutOfBounds, ObstacleCollision };
pub const MoveConst = struct {
    pub const Visited: u8 = 'X';
...
pub fn findPosOfGuard(array_of_lines: *lines) ?GuardPos {
...
pub fn RotateRight(direction: Direction) Direction {
    switch (direction) {
        .Up => return .Right,
        .Down => return .Left,
        .Left => return .Up,
...
pub fn TraceSoldierPath(buffer: *string, array_of_lines: *lines) !u64 {
    ...
    buffer.* = string.fromOwnedSlice(allocator, try fstream.readToEndAlloc(allocator, size_of_file));
    array_of_lines.* = lines.fromOwnedSlice(allocator, try splitByLine.convertToSliceOfLines(allocator, buffer));

    const inital_pos = findPosOfGuard(array_of_lines).?;
    var pos = inital_pos;
    ...
    while (MoveSoldier(array_of_lines, &pos, direction) != .OutOfBounds) {
        ...
        array_of_lines.items[inital_pos.line][inital_pos.col] = MoveConst.Guard;
        array_of_lines.items[pos.line][pos.col] = MoveConst.Visited;
    }

    return DistinctPosVisited(buffer);
}
